...
import { getBrowserRpcClient } from '@/core/crypto/transactions/transactionBuilder'
import {
  TokenMetadata,
  TokenBalanceItem,
  CustomTokenRecord,
  standardErc20Abi,
} from '@/core/crypto/tokens/tokenTypes'

export type { TokenBalanceItem, TokenMetadata }
import { getVerifiedTokensForChain } from '@/core/crypto/tokens/tokenRegistry'
import {
  getCustomTokens,
  saveCustomToken,
  deleteCustomToken,
} from '@/core/crypto/tokens/tokenStorage'
...
export async function fetchUserTokensAndBalances(
  ...
  let linkUsd = 14.50
  try {
    const market = await orpc.prices.getPrices({})
