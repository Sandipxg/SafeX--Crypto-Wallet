...
import { getTokenMetadata } from '../read/tokens.js'
...
export interface SwapQuoteParams {
  ...
  slippageTolerancePercent?: number // e.g. 0.5 for 0.5%
  chainId?: number
}
...
/**
 * Uniswap V2 AMM Constant-Product Formula ($x \cdot y = k$) with 0.30% fee
 */
 ...
 */
export function calculateSlippageBound(
  expectedAmountOut: bigint,
  slippageTolerancePercent: number = 0.5
): bigint {
  if (expectedAmountOut <= 0n) return 0n
  // Convert percentage to basis points (e.g. 0.5% -> 50 bps out of 10,000)
...
export function calculatePriceImpact(
  ...
  // Spot price = reserveOut / reserveIn (scaled by 1e18)
...
async function resolveFastTokenMetadata(
  address: `0x${string}`,
  isNative: boolean,
  chainId: number
...
function cleanDecimalAmount(val: string | undefined): string | null {
  if (!val) return null
...
export async function fetchSwapQuote(
  ...
  // Fetch token metadata with 0-latency registry lookup first
  ...
  // Determine path: direct or via WETH
  ...
  let calculatedImpact = 0.30 // Uniswap V2 0.30% fee base impact
  ...
  if (isExactOut) {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Router timeout')), 7500)
      )
    ...
    try {
      ...
      // Accurate price impact & pool deviation calculation
      try {
        // 1. Marginal spot rate check with a micro-trade to measure AMM slippage
      ...
      // 2. Global benchmark reference comparison for depegged or drained testnet pools
